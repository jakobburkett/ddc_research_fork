...
def load_sp_model(sp_ckpt_fp, sess, batch_size=128):
    with tf.variable_scope('model_sp'):
        model_sp = OnsetNet(
            ...
            zack_hack=0)
    filtered_vars = filter(lambda v: 'model_sp' in v.name, tf.global_variables())
    model_sp_vars = list()
    ...
    # saver.restore(sess, './infer/tmp/train/onset_net_early_stop_fscore-20000')
    saver.restore(sess, './infer/server_aux/model_sp-56000')
    # saver.restore(sess, './learn/tmp/train_perplexity/onset_net_train-52000')
    return model_sp
...
def load_ss_model(ss_ckpt_fp, sess):
    ...
    # sess.run(init)
    filtered_vars = filter(lambda v: 'model_ss' in v.name, tf.global_variables())
    model_ss_vars = list()
    for v in filtered_vars:
    ...
    # end of model_ss_vars
    
    
    
    # """)
    # print(model_ss_vars['model_ss/rnn_unroll/MultiRNNCell/Cell1/BasicLSTMCell/Linear/Bias'])
    # from tensorflow.python.tools.inspect_checkpoint import print_tensors_in_checkpoint_file
    # print_tensors_in_checkpoint_file(file_name='./tmp/mdl_ckpt', tensor_name='', all_tensors=False)
    # model_ss_vars.append('Tensor("model_ss/rnn_unroll/multi_rnn_cell/cell_0/basic_lstm_cell/bias/read:0", shape=(256, 512), dtype=float32)')
...
diff_to_coarse_fine_threshold = {
    'Beginner':     (0, 1, 0.45),     #ddc: 0.15325437    mine: 0.4185849
    'Easy':         (1, 3, 0.445),   #ddc: 0.23268291    mine: 0.44
    'Medium':         (2, 5, 0.435),  #ddc: 0.29456162    mine: 0.435
    'Hard':         (3, 7, 0.43),
    'Challenge':    (4, 9, 0.42)}
...
def create_chart_dir(
    ...
    for diff in diffs:
        try:
            coarse, fine, threshold = diff_to_coarse_fine_threshold[diff]
        except KeyError:
...
def change_thresholds():
    beginner_threshold = request.form.get('beginner_threshold')
    easy_threshold = request.form.get('easy_threshold')
    medium_threshold = request.form.get('medium_threshold')
    hard_threshold = request.form.get('hard_threshold')
    challenge_threshold = request.form.get('challenge_threshold')

    if beginner_threshold != '' and beginner_threshold > 0:
        diff_to_coarse_fine_threshold['Beginner'] = (0, 1, float(beginner_threshold))

    if easy_threshold != '' and easy_threshold > 0:
        diff_to_coarse_fine_threshold['Easy'] = (1, 3, float(easy_threshold))

    if medium_threshold != '' and medium_threshold > 0:
        diff_to_coarse_fine_threshold['Medium'] = (2, 5, float(medium_threshold))

    if hard_threshold != '' and hard_threshold > 0:
        diff_to_coarse_fine_threshold['Hard'] = (3, 7, float(hard_threshold))

    if challenge_threshold != '' and challenge_threshold > 0:
        diff_to_coarse_fine_threshold['Challenge'] = (4, 9, float(challenge_threshold))

    print("New thresholds:")
    print(diff_to_coarse_fine_threshold)
