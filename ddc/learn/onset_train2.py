...
tf.compat.v1.app.flags.DEFINE_bool('z_score', False, 'If true, train and test on z-score of training data')
...
tf.compat.v1.app.flags.DEFINE_string('export_feat_name', '', 'If set, export CNN features to this directory')

# Features
tf.compat.v1.app.flags.DEFINE_integer('audio_context_radius', 7, 'Past and future context per training example')
tf.compat.v1.app.flags.DEFINE_integer('audio_nbands', 80, 'Number of bands per frame')
tf.compat.v1.app.flags.DEFINE_integer('audio_nchannels', 3, 'Number of channels per frame')
tf.compat.v1.app.flags.DEFINE_string('audio_select_channels', '', 'List of CSV audio channels. If non-empty, other channels excluded from model.')
...
tf.compat.v1.app.flags.DEFINE_bool('cnn_rnn_zack', False, '')
tf.compat.v1.app.flags.DEFINE_integer('zack_hack', 0, '')
...
tf.compat.v1.app.flags.DEFINE_integer('rnn_nunroll', 1, '')
...
tf.compat.v1.app.flags.DEFINE_string('dnn_nonlin', 'sigmoid', '')

# Training params
tf.compat.v1.app.flags.DEFINE_integer('batch_size', 256, 'Batch size for training')
tf.compat.v1.app.flags.DEFINE_string('weight_strategy', 'rect', 'One of \'rect\' or \'last\'')
tf.compat.v1.app.flags.DEFINE_bool('randomize_charts', False, '')
tf.compat.v1.app.flags.DEFINE_bool('balanced_class', True, 'If true, balance classes, otherwise use prior.')
tf.compat.v1.app.flags.DEFINE_integer('exclude_onset_neighbors', 0, 'If nonzero, excludes radius around true onsets from dataset as they may be misleading true neg.')
tf.compat.v1.app.flags.DEFINE_bool('exclude_pre_onsets', False, 'If true, exclude all true neg before first onset.')
tf.compat.v1.app.flags.DEFINE_bool('exclude_post_onsets', False, 'If true, exclude all true neg after last onset.')
...
# Eval params
tf.compat.v1.app.flags.DEFINE_string('eval_window_type', '', '')
tf.compat.v1.app.flags.DEFINE_integer('eval_window_width', 0, '')
tf.compat.v1.app.flags.DEFINE_integer('eval_align_tolerance', 2, '')
tf.compat.v1.app.flags.DEFINE_string('eval_charts_export', '', '')
tf.compat.v1.app.flags.DEFINE_string('eval_diff_coarse', '', '')
...
def main(_):
    ...
    model_config = {
        ...
        'cnn_init': tf.compat.v1.keras.initializers.VarianceScaling(scale=1.43, dtype=dtype, distribution="uniform"),
        ...
        'dnn_init': tf.compat.v1.keras.initializers.VarianceScaling(scale=1.15, dtype=dtype, distribution="uniform"),
...
def eval_metrics_for_scores(y_true, y_scores, y_xentropies, y_scores_pkalgn, given_threshold_names=[], given_thresholds=[], return_prc=False):
    ...
    # calculate accuracy
    y_labels = np.zeros(y_scores_pkalgn.shape[0], dtype=np.int64)
    y_labels[predicted_steps] = 1
    accuracy = accuracy_score(y_true.astype(np.int64), y_labels)
